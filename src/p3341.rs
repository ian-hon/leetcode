use std::{cmp::Reverse, collections::{BinaryHeap, HashSet}, time::Instant};
...
pub fn run() {
    for i in [
        ...
        vec![
            vec![367,461,181,437,448,376, 60],
            vec![294,  9,403,231,269,297, 39],
            vec![  9,239,276,355,318, 13,135]
        ] // 359
    ] {
...
pub fn min_time_to_reach(move_time: Vec<Vec<i32>>) -> i32 {
    ...
    let mut nodes: BinaryHeap<Reverse<(i32, (usize, usize))>> = BinaryHeap::new();
    nodes.push(Reverse((0, (0, 0))));
    traversed[0][0] = true;

    // current node -> (cost, pos, current_time)
    // current_time includes current pos

    while let Some(Reverse((time, pos))) = nodes.pop() {
        for (x, y) in get_neighbours(pos, height, width, &traversed) {
            let payment = (move_time[y][x] - time).max(0) + 1;

            if (x, y) == target {
                return time + payment;
            }
            nodes.push(Reverse((time + payment, (x, y))));
            traversed[y][x] = true;
        }
    }
